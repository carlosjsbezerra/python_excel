...
for row in ws_escolas.iter_rows(min_row=2, values_only=True):
    ...
    if valor_coluna_c == 'TOCANTINOPOLIS':  # Verificar se o valor na coluna "C" é "TOCANTINOPOLIS"
        linhas_para_copiar.append(row)
        print("Linha encontrada:", row)

# Verificar se alguma linha foi encontrada
if not linhas_para_copiar:
    print("Nenhuma linha encontrada com o valor 'TOCANTINOPOLIS' na coluna C.")

# Adicionar as linhas filtradas começando na célula A11 da subplanilha "SRE TOCANTINOPOLIS"
start_row = 11
for i, linha in enumerate(linhas_para_copiar):
    for j, value in enumerate(linha):
        ws_sre_tocantinopolis.cell(row=start_row + i, column=j + 1, value=value)

# Verificar se as linhas foram adicionadas
if linhas_para_copiar:
    print(f"{len(linhas_para_copiar)} linhas copiadas para 'SRE TOCANTINOPOLIS'.")

# Salvar as alterações na planilha "planilha002"
wb_planilha002.save(filename='Pasta002/planilha002.xlsx')

print("Linhas copiadas com sucesso para a subplanilha 'SRE TOCANTINOPOLIS'.")