...
from teste_ZaA import organizar_coluna_f
...
def copiar_linhas_arraias():
    ...
    for row in ws_escolas.iter_rows(min_row=2, values_only=True):
        ...
        if valor_coluna_c == 'ARRAIAS':  # Verificar se o valor na coluna "C" é "ARRAIAS"
            linhas_para_copiar.append(row)
            print("Linha encontrada:", row)

    # Verificar se alguma linha foi encontrada
    if not linhas_para_copiar:
        print("Nenhuma linha encontrada com o valor 'ARRAIAS' na coluna C.")

    # Adicionar as linhas filtradas começando na célula A11 da subplanilha "SRE ARRAIAS"
    start_row = 11
    for i, linha in enumerate(linhas_para_copiar):
        for j, value in enumerate(linha):
            ws_sre_arraias.cell(row=start_row + i, column=j + 1, value=value)
