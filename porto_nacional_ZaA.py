...
def organizar_coluna_f():
    ...
    for row in ws_sre_porto_nacional.iter_rows(min_row=start_line, max_row=finish_line, min_col=6, max_col=6, values_only=True):
    ...
    for i, valor in enumerate(valores_coluna_f):
        ws_sre_porto_nacional.cell(row=11 + i, column=6, value=valor)

    # Salvar as alterações na planilha "planilha002"
    wb_planilha002.save(filename='PIEC_2024_v2/lista_escola_selecionada.xlsx')

    print("Coluna 'F' organizada com sucesso na subplanilha 'SRE PORTO NACIONAL' da linha 11 até a linha 26.")
