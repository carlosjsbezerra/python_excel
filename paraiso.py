...
def copiar_linhas_paraiso():
    ...
    ws_sre = wb_planilha002['SRE PARAISO']
    ...
    for row in ws_escolas.iter_rows(min_row=2, values_only=True):
        ...
        if valor_coluna_c == 'PARAISO':  # Verificar se o valor na coluna "C" é o procurado
            linhas_para_copiar.append(row)
            print("Linha encontrada:", row)

    # Verificar se alguma linha foi encontrada
    if not linhas_para_copiar:
        print(f"Nenhuma linha encontrada com o valor 'PARAISO' na coluna C.")
    ...
    print(f"Linhas copiadas com sucesso para a subplanilha 'SRE PARAISO'.")
